...
class Login
{
    ...
    private function loginWithPostData()
    {
        ...
        if (!empty($_POST['user_name']) && !empty($_POST['user_password'])) {
            ...
            if (!$this->db_connection->connect_errno) {
                ...
                $checklogin = $this->db_connection->query("SELECT user_name, user_email, user_password_hash FROM user WHERE user_name = '" . $this->user_name . "';");
                ...
                if ($checklogin->num_rows == 1) {
                    ...
                    if (password_verify($_POST['user_password'], $result_row->user_password_hash)) {
                        ...
                        $this->errors[] = '<div class="logout_message alert alert-danger">סיסמא שגויה.נסו שנית</div>';
                        $_SESSION['error_pass'] = 'error_pass';
                    }
                } else {
                    $this->errors[] = '<div class="logout_message alert alert-danger">חשבון לא קיים</div>';
            ...
            $this->errors[] = "שדה של שם משתמש ריק";
        } elseif (empty($_POST['user_password'])) {
            $this->errors[] = "שדה של סיסמה ריק";
        }
    }
    ...
    public function doLogout()
    {
       ...
       // unset($_SESSION['user_name']);
        ...
        $this->messages[] = '<div class="logout_message alert alert-success">יצאת מהמערכת.</div>';
